...
// camelCase ve snake_case her ikisini destekle
...
function renderPrograms(programs) {
  ...
  container.innerHTML = programs.map(p => {
    const isActive     = g(p, 'isActive', 'is_active')
    const exCount      = p.exercises?.length ?? g(p, 'exerciseCount', 'exercise_count') ?? 0
    return `
    <div class="program-card ${isActive ? 'is-active' : ''}">
      <div class="program-card-header">
        <div>
          <h3>
            ${p.name}
            ${isActive ? '<span class="badge badge-active" style="margin-left:8px">Active</span>' : ''}
          </h3>
          <p style="color:var(--text-3);font-size:0.8rem;margin-top:4px">${exCount} exercises</p>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
          <button class="${isActive ? 'btn-active-prog' : 'btn-secondary'}"
...
function searchProgEx(progId) {
  ...
  const filtered = allExercises
    .filter(e => e.name.toLowerCase().includes(q) || (e.muscleGroup||e.muscle_group||'').toLowerCase().includes(q))
    .slice(0, 8)
  if (!filtered.length) {
  ...
  drop.innerHTML = filtered.map(e => `
    <div onclick="pickProgEx('${progId}',${e.id},'${e.name.replace(/'/g,"\\'")} (${e.muscleGroup||e.muscle_group})')"
      ...
      <span class="badge badge-gold" style="font-size:0.65rem">${e.muscleGroup||e.muscle_group}</span>
    </div>
  `).join('')
  drop.classList.remove('hidden')
}
...
async function loadProgExList(progId) {
  ...
  try {
    ...
    exercises.forEach(ex => {
      const day = g(ex, 'dayOfWeek', 'day_of_week') || 1
    ...
    container.innerHTML = Object.keys(byDay).sort().map(day => `
      <div class="day-col" style="margin-bottom:14px">
        <h4>${days[day]}</h4>
        ${byDay[day].map(ex => {
          const exName   = g(ex.exercise || ex, 'name', 'exercise_name') || ex.exerciseName || 'Unknown'
          const sets     = ex.sets
          const reps     = g(ex, 'targetReps', 'target_reps')
          return `
          <div class="day-ex-item">
            <div>
              <strong style="font-size:0.85rem">${exName}</strong>
              <span style="margin-left:6px">${sets}×${reps}</span>
            </div>
...
async function createProgram() {
  ...
  try {
    await api('/programs', 'POST', { name, goal: 'strength', daysPerWeek: 3 })
...
async function addExToProgram(progId) {
  ...
  try {
    await api(`/programs/${progId}/exercises`, 'POST', {
      exerciseId: parseInt(exId), dayOfWeek: day, sets, targetReps: reps
    })
...
async function loadWorkoutTab() {
  ...
  try {
    ...
    const active   = programs.find(p => g(p, 'isActive', 'is_active'))
    if (!active) {
      container.innerHTML = `
    ...
    const exCount = active.exercises?.length ?? g(active, 'exerciseCount', 'exercise_count') ?? 0
    container.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div>
          <h3 style="margin-bottom:4px">${active.name}</h3>
          <p style="color:var(--text-3);font-size:0.82rem">${exCount} exercises</p>
        </div>
...
async function startWorkout(progId) {
  try {
    ...
    const session   = await api('/sessions', 'POST', { sessionDate, programId: progId })
    activeSessionId = session.id

    const prog    = await api(`/programs/${progId}`)
    const todayNum = today.getDay() === 0 ? 7 : today.getDay()
    const allEx    = prog.exercises || []
    const todayEx  = allEx.filter(e => g(e, 'dayOfWeek', 'day_of_week') === todayNum)
    ...
    if (todayEx.length === 0) {
      showConfirm(
        ...
        () => {
          workoutExercises = allEx.map(ex => ({ ...ex, exerciseId: ex.exerciseId || ex.exercise_id || ex.id }))
          failureState = {}
    ...
    workoutExercises = todayEx.map(ex => ({ ...ex, exerciseId: ex.exerciseId || ex.exercise_id || ex.id }))
    failureState = {}
    showActiveWorkout(prog, today)
    if (!allExercises.length) allExercises = await api('/exercises')
    lucide.createIcons()
...
function renderWorkoutExercises(exercises) {
  ...
  exercises.forEach((ex, i) => {
    const d = g(ex, 'dayOfWeek', 'day_of_week') || 1
    if (!byDay[d]) byDay[d] = []
  ...
  const contentsHtml = days.map((day, di) => `
    <div id="day-content-${day}" class="day-content ${di===0?'':'hidden'}">
      ${byDay[day].map(ex => {
        const exName  = ex.exercise?.name || g(ex, 'exerciseName', 'exercise_name') || 'Unknown'
        const muscle  = ex.exercise?.muscleGroup || ex.exercise?.muscle_group || g(ex, 'muscleGroup', 'muscle_group') || ''
        const sets    = ex.sets
        const reps    = g(ex, 'targetReps', 'target_reps')
        return `
        <div class="workout-ex-card">
          ...
          <div class="workout-ex-meta">
            ...
            <span>${sets} set · ${reps} target reps</span>
          </div>
          <div class="workout-inputs">
            ...
            <div class="form-group">
              <label>Reps</label>
              <input type="number" id="w-rep-${ex.idx}" placeholder="${reps}" min="1">
            </div>
            <div class="form-group">
              <label>Failure?</label>
  ...
  exercises.forEach((ex, i) => loadPrevPerf(i, ex.exerciseId || g(ex, 'exerciseId', 'exercise_id')))
  lucide.createIcons()
}
...
async function loadPrevPerf(index, exId) {
  try {
    ...
    for (const s of sessions) {
      ...
      const prev   = detail.exercises?.find(e => (e.exerciseId || e.exercise_id) === exId)
      if (prev) {
        const input = document.getElementById(`w-kg-${index}`)
        const wkg   = g(prev, 'weightKg', 'weight_kg')
        if (input && !input.value) {
          input.value        = wkg
          input.dataset.prev = wkg
          input.placeholder  = `Prev: ${wkg}kg`
        }
        return
      }
    }
...
async function finishWorkout() {
  ...
  for (let i = 0; i < workoutExercises.length; i++) {
    ...
    const exId = ex.exerciseId || g(ex, 'exerciseId', 'exercise_id')
    if (!exId) { errors.push(`${ex.exercise?.name || 'Unknown'}: missing ID`); skipped++; continue }
    try {
      await api(`/sessions/${activeSessionId}/exercises`, 'POST', {
        exerciseId: exId, weightKg: weight, reps, reachedFailure: failure
      })
      logged++
    } catch (err) { errors.push(`${ex.exercise?.name}: ${err.message}`); skipped++ }
  }
...
function renderHistory(sessions) {
  ...
  container.innerHTML = sessions.map(s => {
    const date     = g(s, 'sessionDate', 'session_date')
    const progName = s.program?.name || g(s, 'programName', 'program_name') || 'Manual workout'
    const exCount  = s.exercises?.length ?? g(s, 'exerciseCount', 'exercise_count') ?? 0
    return `
    <div class="program-card">
      <div class="program-card-header" style="cursor:pointer" onclick="toggleHistory(${s.id})">
        <div>
          <h3 style="font-size:0.92rem;margin-bottom:4px">${formatDate(date)}</h3>
          <p style="color:var(--text-3);font-size:0.8rem">
            ${progName} · ${exCount} exercises
          </p>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
...
async function loadHistoryDetail(sessionId, container) {
  try {
    ...
    const date     = g(session, 'sessionDate', 'session_date')
    ...
    container.innerHTML = `
      <div class="log-table">
        ...
        ${exercises.map(ex => {
          const name   = ex.exercise?.name || g(ex, 'exerciseName', 'exercise_name') || 'Unknown'
          const weight = g(ex, 'weightKg', 'weight_kg')
          const reps   = ex.reps
          return `
          <div class="log-row">
            <div class="log-left">
              <span class="log-date">[${formatDate(date)}]</span>
              <span class="log-name">${name.toUpperCase()}</span>
            </div>
            <div class="log-right">
              <div class="log-load">
                <span class="log-kg">${weight} <small>KG</small></span>
                <span class="log-reps">× ${reps} <small>REPS</small></span>
              </div>
            </div>
          </div>
        `}).join('')}
